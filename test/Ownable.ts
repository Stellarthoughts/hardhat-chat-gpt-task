...
describe("Ownable", function () {
	...
	describe("Deployment", function () {
		it("Should set the correct owner", async function () {
			const { testOwnable, owner } = await loadFixture(
	...
	describe("Modifiers", function () {
		it("Should revert if non-owner calls the function with onlyOwner", async function () {
			const { testOwnable, otherAccount } = await loadFixture(
				deployContractFixture
			);
			await expect(
				testOwnable.connect(otherAccount).withOnlyOwner()
			).to.be.revertedWith("Only owner can access this function");
		});
	});
});
