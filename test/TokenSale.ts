...
describe("TokenSale", function () {
  ...
  // Deployments
  async function deployContractCustom(
    timeStart: number,
    timeEnd: number,
  ...
  describe("Buying", function () {
    ...
    it("Should keep track of invested ethers", async function () {
      ...
      expect(await tokenSale.ownerToEthers(otherAccount.address)).to.equal(
        ethersAmount
      );
    });
  });
  ...
  describe("Distribution", function () {
    ...
    it("Should distribute correctly among 3 investors", async function () {
      ...
      for (let i = 0; i < buyers.length; i++) {
        expect(await tokenSale.ownerToTokens(buyers[i].address)).to.equal(
          expectedTokens
        );
      }
    });
