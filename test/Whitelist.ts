...
describe("Whitelist", function () {
  ...
  describe("Whitelisting", function () {
    it("Should contain whitelisted account and be true after setWhitelisted to true", async function () {
      const { testWhitelist, otherAccount } = await loadFixture(
        deployContractFixture
      );
      await testWhitelist.setWhitelisted(otherAccount.address, true);
      expect(await testWhitelist.whitelisted(otherAccount.address)).to.equal(
        true
      );
    });
  });
  ...
  describe("Modifiers", function () {
    it("Should revert if non-whitelisted calls the function with onlyWhitelisted", async function () {
      ...
      await expect(
        testWhitelist.connect(otherAccount).withWhitelist()
      ).to.be.revertedWith("You should be whitelisted to call this function");
    });
  });
});
